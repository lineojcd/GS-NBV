...
class SCNBVPerceiver:
    ...
    def __init__(self):
        ...
        self.equatorial_radius = rospy.get_param("SCNBV_equatorial_radius", "0.02")
        ...
        self.fruit_position_estimation = rospy.get_param("SCNBV_fruit_position_estimation_method")
        self.fruit_vertics_Top = rospy.get_param("SCNBV_fruit_vertics_Top")
        self.fruit_vertics_Left = rospy.get_param("SCNBV_fruit_vertics_Left")
        self.fruit_vertics_Right = rospy.get_param("SCNBV_fruit_vertics_Right")
    ...
    def run(self):
        ...
        if camera_info is None or color_image is None or depth_image is None:
            rospy.logwarn("Make sure you handle the semantic_res_dict None issue ... ")
        ...
        if seg_masks is not None:
            ...
            occlusion_rate = 1.0
            occluded_pts = None
        rospy.loginfo(f"picking_condition is {picking_condi}, valid occlusion rate is: {occlusion_rate},  compute occlusion rate takes: {round((time.time() - start_time),4)} seconds")
    ...
    def get_contour_envelope_min_max_depth(self, contour_depth_v_set, envelope_depth_v_set):
        try:
            if self.filter_depth:
                ...
                if len(contour_depth_v_set) == 0:
                    fruit_min_depth = None
                    fruit_max_depth = None
                else:
                    fruit_min_depth = round(min(contour_depth_v_set),4)
                    fruit_max_depth = round(max(contour_depth_v_set),4)
                if len(envelope_depth_v_set) == 0:      
                    enve_min_depth = None
                    enve_max_depth = None              
                else:
    ...
    def get_occlusion_rate_and_points(self,depth_image, seg_masks):
        ...
        valid_cont_minD, valid_cont_maxD, valid_env_minD, valid_env_maxD = self.get_contour_envelope_min_max_depth(valid_cont_depth_values_set,valid_env_depth_values_set)
    ...
    def estimate_picking_point(self,depth_img, keypt_dict, cam_pose, device = torch.device("cuda:0")):
        """
        Get fruit pose
        """
        position = torch.tensor(cam_pose[:3], dtype=torch.float32, device=device)
        ...
        fruit_idx = keypt_dict[self.fruit_position_estimation]            # fruit_body or fruit_contour
        PC = self.get_fruit_centroid_pixel(fruit_idx)
        T = keypt_dict[self.fruit_vertics_Top]                # fruit_top
        fruit_left_idx = keypt_dict[self.fruit_vertics_Left]               # fruit_left
        fruit_right_idx = keypt_dict[self.fruit_vertics_Right]              # fruit_right
        w = fruit_right_idx[0] - fruit_left_idx[0]
        D_pixel_idx = torch.tensor([PC[0], T[1] - 3*w/8]).round().long()
        Q_est = self.estimate_point_position(pixel_coords, Trans_w_o, depth_img, PC, D_pixel_idx, batch_size)
        
        # if self.verbose_perceiver:
        print("PC pixel is:",PC)
        print("w is:",w)
        print("D is:",D_pixel_idx)
        print("Q_est is ",Q_est)
        return Q_est
    
    def get_fruit_centroid_pixel(self, tensor):
        return torch.mean(tensor.float(), dim=0).round().int()
    
    def estimate_point_position(self, pixel_coords, cam_transform_in_world, depth_img, depth_pixel, point_idx, batch_size, verbose=False):
        row, col = point_idx
        PC_row, PC_col = depth_pixel
        Zc =  depth_img[PC_row, PC_col].view(1, -1)
        Zd = Zc + self.equatorial_radius
        
        point_pixel_coords = pixel_coords[row, col].view(-1, 3) @ torch.inverse(self.intrinsics).t().type(torch.float32)
        point_cam_coords = (point_pixel_coords.flatten() * Zd).view(batch_size, -1, 3)
        point_world_coords = transform_points_cam_to_world_my(point_cam_coords, cam_transform_in_world, self.arm_type).cpu()
        
        # if self.verbose_perceiver:
        print("Zd is ",Zd)
        print("point_pixel_coords ", point_pixel_coords)
        print("point_cam_coords ", point_cam_coords)
