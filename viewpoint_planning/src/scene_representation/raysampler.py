...
class RaySampler:
    ...
    def ray_origins_directions(
        ...
        # log_odds(0.9)  # 2.2 occupied_odds | log_odds(0.4)  # -0.4 free | log_odds(0.5)  # 0
        points_mask = torch.where(max_depths < self.z_far, 2.2, -0.4)
        
        # Transform the camera coordinates to world coordinates: camera coordinates: [x, y, 1]
        # TODO: Do I need to remove this grad?
        ...
        # ray_origins shape: torch.Size([1, 270000, 3])
        # ray_targets shape: torch.Size([1, 270000, 3])
        # ray_directions shape: torch.Size([1, 270000, 3])
...
if __name__ == "__main__":
    ...
    depth_image = torch.ones((1, width, height), dtype=torch.float32, device=sampler.device)
