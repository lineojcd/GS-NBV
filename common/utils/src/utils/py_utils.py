...
def get_arm_pose_ori(posi_w_cam, R_w_cam, R_cam_ee):
    R_w_cam = R.from_quat(R_w_cam)
    R_w_ee = R_w_cam * R_cam_ee
    arm_pose = np.concatenate((posi_w_cam, R_w_ee.as_quat()))  # Quaternion form
    return arm_pose

def get_arm_pose(posi_w_o, R_w_o, R_cam_ee, R_o_cam):
    R_w_o = R.from_quat(R_w_o).as_matrix()
    R_o_cam = R.from_quat(R_o_cam).as_matrix()
    R_cam_ee = R.from_quat(R_cam_ee).as_matrix()
    R_w_ee = R_w_o @ R_o_cam @ R_cam_ee
    arm_pose = np.concatenate((posi_w_o, R.from_matrix(R_w_ee).as_quat()))  # Quaternion form
    return arm_pose


def my_pause_program():
    user_input = input("Enter 'quit'to exit the program, or press any key to resume...")
