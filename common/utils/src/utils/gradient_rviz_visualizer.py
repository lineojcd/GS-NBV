...
class GradientRvizVisualizer:
    def __init__(self):
        ...
        self.picking_ring_line_width = rospy.get_param('picking_ring_line_width', "0.004")
        self.picking_ring_color = [float(x) for x in rospy.get_param('picking_ring_color').split()]
        ...
        self.gnbv_viewpoint_color = [float(x) for x in rospy.get_param('nbv_viewpoint_color').split()]
        self.gnbv_viewpoint_scale = [float(x) for x in rospy.get_param('GNBV_viewpoint_scale').split()]
        self.nbv_viewpoint_color = [float(x) for x in rospy.get_param('nbv_viewpoint_color').split()]
        self.nbv_viewpoint_scale = [float(x) for x in rospy.get_param('GNBV_viewpoint_scale').split()]
        self.nbv_viewpoint_length_coef = rospy.get_param('GNBV_viewpoint_length_coef', "0.15")
        ...
        self.gnbv_viewpoint_pub = rospy.Publisher("gnbv_viewpoint", Marker, queue_size=1)
    ...
    def visualize_viewpointslist(self, viewpoints, fruit_position, viewpoint_type):
        if len(viewpoints)==0:
            ...
            return 0
        # self.clear_markers(self.previous_viewpoints_pub)
        # self.clear_markers(self.new_viewpoints_pub)
        # self.clear_markers(self.reachable_viewpoints_pub)
        # self.clear_markers(self.filtered_viewpoints_pub)
        # self.clear_markers(self.valid_viewpoints_pub)
        marker_array = MarkerArray()
    ...
    def clear_markers(self, publisher):
        """
        Publishes a DELETEALL marker, which is the cleanest way to clear all markers if you don't track IDs.
        """
    ...
    def visualize_voxels(self, points: np.array, class_ids: np.array = None) -> None:
        """
    ...
    def visualize_picking_ring_GT(self, resolution=100): 
        ...
        marker.type = Marker.LINE_STRIP
        marker.action = Marker.ADD
        marker.scale.x = self.picking_ring_line_width  # Line width
        marker.color.a = self.picking_ring_color[3]  # Don't forget to set the alpha!
    ...
    def visualize_viewpoint(self, viewpoint: Pose) -> None:
        ...
        color = self.gnbv_viewpoint_color
        scale = self.gnbv_viewpoint_scale
        # marker.scale = Vector3(0.08, 0.01, 0.01)
        marker.scale = Vector3(scale[0], scale[1], scale[2])   # x Shaft diameter, y Head diameter, z Head length 
        marker.color = ColorRGBA(color[0], color[1], color[2], color[3])
        self.gnbv_viewpoint_pub.publish(marker)
