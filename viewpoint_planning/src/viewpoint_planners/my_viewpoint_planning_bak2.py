...
class MyViewpointPlanning:
    def __init__(self):
        ...
        self.fruit_pose_update_rate = rospy.get_param("fruit_pose_update_rate", "0.70")    
        # self.fruit_position_last = np.array([0.0, 0.0, 0.0])
        # self.fruit_axis_last = np.array([0.0, 0.0, 0.0])
        self.fruit_position_last = None
        self.fruit_axis_last = None
    ...
    def config_init_pose(self):
        ...
        posi = np.array([self.init_tar_posi[0], self.init_tar_posi[1] + self.init_cam_dist, self.init_tar_posi[2]])
        ...
        # test
        # pose = np.array([0.20458, -0.3214, 0.43691, 0.10512, -0.68457, -0.12612, 0.71022])
        # print("pose is : ", pose)
        # print("self.arm_pose is : ",self.arm_pose)
        # is_success = self.arm_control.move_arm_to_pose(pose)
        
        # pose = np.array([0.20458, -0.3214, 0.43691, 0.10512, -0.68457, -0.12612, 0.71022])
        # pose = np.array([0.2222,-0.35043,0.46692,0.10286,-0.6816,-0.1285,0.71297])
        # self.arm_pose = pose
    ...
    def run_mynbv(self):
        ...
        if self.planning_flag:
            ...
            if self.picking_score >= self.picking_threshold:        # Staging the fruit
                my_pause_program()
                self.fruit_staging()
            else:
                my_pause_program()
    ...
    def update_fruit_pose(self, fruit_position, fruit_axis):
        if self.fruit_axis_last is not None:
            self.fruit_position = self.fruit_position_last + self.fruit_pose_update_rate * (fruit_position - self.fruit_position_last)
            self.fruit_axis = self.fruit_axis_last + self.fruit_pose_update_rate * (fruit_axis - self.fruit_axis_last)
        else:
            self.fruit_position = fruit_position
            self.fruit_axis = fruit_axis
        
        if self.verbose_planning:
            print("type of current iter fruit_position", type(fruit_position))   # ndarray
            print("type of fruit_posi_GT", type(self.fruit_posi_GT))             # list
            print("GT fruit position:", self.fruit_posi_GT)
        print("Last iter fruit position and axis are:", self.fruit_position_last, self.fruit_axis_last)
        print("current iter Est. fruit position and axis are:", fruit_position, fruit_axis)
        print("current iter Est. error of fruit position:", fruit_position - self.fruit_posi_GT)
        print("weighted current iter Est. fruit position and axis are:", self.fruit_position, self.fruit_axis)
        print("weighted current iter Est. error of fruit position:", self.fruit_position - self.fruit_posi_GT)
        
        self.fruit_position_last = self.fruit_position.copy()
        self.fruit_axis_last = self.fruit_axis.copy()
    ...
    def sensing(self):
        ...
        fruit_position, fruit_axis = self.perceiver.estimate_fruit_pose(self.depth_img,self.keypoint_dict,self.optical_pose)  
        self.update_fruit_pose(fruit_position, fruit_axis)
    ...
    def update_semantic_octomap(self):
        ...
        self.roi_coverage = self.mynbv_planner.update_semantic_octomap(self.depth_img, self.semantics, self.optical_pose)
    ...
    def fruit_staging(self):
        staging_optical_pose = self.mynbv_planner.get_staging_pose(self.fruit_position, self.optical_pose)
